...
%         \hline
%         \textbf{Model} & \textbf{mAP\raisebox{-1pt}{\textsuperscript{50-95}}} & \textbf{CPU ONNX (ms)} & \textbf{Params (M)} & \textbf{FLOPs (B)} \\
%         \hline
...
%     \end{tabular}
%     }
%     \caption{YOLOv8 Model Performance \cite{yolov8}}
%     \label{tab:yoloperformance}
% \end{table}

% For regular AABB (axis-aligned bounding box) detection, the YOLOv8 model trained on the COCO dataset \cite{cocodataset} would provide a good starting point. The COCO dataset contains 80 classes, and the performance of different sized YOLO models is shown in \autoref{tab:yoloperformance}.

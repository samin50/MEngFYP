\section{Safety Plan}
As the system is an mechanical and electrical system, there are a number of safety considerations that need to be taken into account.
\subsection{Electrical Safety}
As mentioned in Section \ref*{sec:hardware}, the system is powered by a 24V, 6.25A, 150W power supply, and must be stepped down to 5V for the Pi and 12V for the LED strip. This is achieved using a XL4015 Step Down Converter\cite{xl4015}, a variable step down converter that can output up to 75W.
...
A standard IEC C14 power connector and a RS Pro Snap-In Fused Rocker Switch\cite{rsproc14switch} is used to connect the PSU to the mains power.
...
for this stage. However, as mentioned in \ref*{sec:projectplan} Stage 2 requires the use of a conveyer belt system, and Stage 3 requires the use of a vibratory bowl feeder.
In this stages mechanical safety will be a key consideration, and will be discussed in the relevant sections in the final report.